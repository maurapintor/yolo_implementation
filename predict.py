...
def main():
    ...
    MAX_MODEL_FILE = max(
        filter(lambda x: x.endswith(".pth.tar"), os.listdir(".")),
        key=lambda x: int(x.split("_")[1])
        )
    print("model found: ", MAX_MODEL_FILE)
    load_checkpoint(torch.load(MAX_MODEL_FILE), model)
    ...
    for batch_idx, (x, y) in enumerate(test_loader):
        ...
        figure, axes = plt.subplots(figsize=(3*x.shape[0], 6), ncols=x.shape[0], nrows=2)
